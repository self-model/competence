...
`Use your keyboard to reveal the letter with as few clicks as possible.`,
	...
	plugin.trial = function(display_element, trial) {
		...
		window.A_position = letter_to_screen_coordinates('A');
		window.Z_position = letter_to_screen_coordinates('Z');

		function screen_to_letter_coordinates(coords) {
			new_x = (coords[0]-window.A_position[0])/(window.Z_position[0]-window.A_position[0]);
			new_y = (coords[1]-window.A_position[1])/(window.Z_position[1]-window.A_position[1]);
		...
		p.draw = function() {
			...
			xy_letter_coords = screen_to_letter_coordinates(xy);
			var hovered_letter = '?'
			for (var i=0; i<alphabet.length; i++) {
				let letter = alphabet[i];
				let xy = letter_to_screen_coordinates(letter)
			...
			thisGame.hover_log.x.push(xy_letter_coords[0]);
			thisGame.hover_log.y.push(xy_letter_coords[1]);
		}
